// AppDeploy: detect BOT_ID and LOCALE from URL params
var _urlP=new URLSearchParams(window.location.search);
var BOT_ID=_urlP.get('bot')||null;
var LOCALE=_urlP.get('locale')||'he';
...
function _waitApi(){
  return new Promise(function(resolve){
    if(window.__api){resolve(window.__api);return}
    document.addEventListener('api-ready',function(){resolve(window.__api)},{once:true});
    setTimeout(function(){resolve(window.__api||null)},3000);
  });
}
...
function safeFetch(url){
  return _waitApi().then(function(api){
    if(api)return api.get(url).then(function(r){return r.data||[];}).catch(function(){return [];});
...
function loadMeta(){
  return Promise.all([
    ...
    safeFetch('/api/labels'),
    safeFetch('/api/teams'),
    safeFetch('/api/inboxes'),
    safeFetch('/api/custom_attributes'),
    safeFetch('/api/contact_fields')
  ]).then(function(r){
...
document.getElementById('savebtn').addEventListener('click',function(){
  ...
  _waitApi().then(function(api){
    if(api)return api.post('/api/bots',data).then(function(r){return r.data});
    return fetchT('/api/bots',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)},30000).then(function(r){if(!r.ok)throw new Error();return r.json()});
  }).then(function(s){
      botData=s;
      if(!BOT_ID){BOT_ID=s.id;history.replaceState(null,'','./bot-editor.html?bot='+s.id)}
...
function loadBot(id){
  _waitApi().then(function(api){
    if(api)return api.get('/api/bots/'+encodeURIComponent(id)).then(function(r){return r.data||null});
    return fetchT('/api/bots/'+encodeURIComponent(id),null,30000).then(function(r){return r.ok?r.json():null});
  }).then(function(d){
